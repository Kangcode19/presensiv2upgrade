    ...
    <script>
        $(function() {
            ...
            if (faceRecognition == 1) {
                ...
                ]).then(startFaceRecognition).catch(err => console.error("Error loading models:", err));
                ...
                async function getLabeledFaceDescriptions() {
                    ...
                    return Promise.all(
                        labels.map(async (label) => {
                            const descriptions = [];
                            for (let i = 1; i <= jmlwajah; i++) {
                                try {
                                    const img = await faceapi.fetchImage(`/storage/uploads/facerecognition/${label}/${i}.png`);
                                    ...
                                    if (detections) {
                                        ...
                                        console.log(2);
                                        console.warn(`Tidak ditemukan wajah dalam ${label}/${i}.png`);
                                        namakaryawan = "unknown";
                                    }
                                } catch (err) {
                                    console.log(3);
                                    console.error(`Error processing image ${label}/${i}.png:`, err);
                                    namakaryawan = "unknown";
                                }
                            }
                ...
                async function startFaceRecognition() {
                    ...
                    const video = document.querySelector('.webcam-capture video'); // Ambil elemen video dari Webcam.js
                    ...
                    setInterval(async () => {
                        ...
                        resizedDetections.forEach((detection) => {
                            ...
                            console.log(faceRecognitionDetected);
                        });
                    }, 100); // Jalankan deteksi wajah setiap 100ms
                }
            }
